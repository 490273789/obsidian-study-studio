...
| Persistence         | `src/core/storage/workbenchStore.ts`, `src/features/flashcards/domain/storage/flashcardRepository.ts`                      | Atomic document serialization and partition persistence in `data.json` via `WorkbenchStore`; FSRS scheduling state, local deck-index cache, and durable session/continuity transitions in `FlashcardRepository` |
...
- ADR-0027: decouple flashcard persistence from core storage; WorkbenchStore owns atomic disk serialization and settings slices, while FlashcardRepository owns flashcard domain persistence.
